...
async function main() {
  ...
  // 9. Deploy Multicall3 (standard address)
  ...
  try {
    const multicallHash = await walletClient.sendTransaction({
      to: multicall3Address as Address.Address,
      data: multicall3Bytecode as `0x${string}`,
    })
    ...
    if (error.message?.includes('already known')) {
      console.log(`   ℹ️  Multicall3 already deployed at: ${multicall3Address}`)
    } else {
      console.log(`   ⚠️  Multicall3 deployment skipped: ${error.message}`)
    }
  }
