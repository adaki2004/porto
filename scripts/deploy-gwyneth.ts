...
const MULTICALL3_ADDRESS = '0xcA11bde05977b3631167028862bE2a173976CA11' as Address.Address
const MULTICALL3_DEPLOYER = '0x05f32B3cC3888453ff71B01135B34FF8e41263F2' as Address.Address
...
async function main() {
  ...
  async function deployContract(args: {
    ...
    if (!receipt.contractAddress) {
      throw new Error(`Failed to deploy ${args.name}`)
    }
  ...
  let multicall3Address = MULTICALL3_ADDRESS
  console.log('Ensuring Multicall3 is deployed at canonical address...')
  const existingMulticallCode = await publicClient.getBytecode({ address: multicall3Address })

  if (existingMulticallCode && existingMulticallCode !== '0x') {
    console.log(`   ✅ Multicall3 already deployed at: ${multicall3Address}`)
  } else {
    console.log('   ℹ️  Multicall3 not found. Funding canonical deployer and broadcasting signed transaction...')
    const deployerBalance = await publicClient.getBalance({ address: MULTICALL3_DEPLOYER })

    if (deployerBalance < MULTICALL3_MIN_BALANCE) {
      const desiredBalance = MULTICALL3_TARGET_BALANCE
      const topUpAmount = desiredBalance > deployerBalance ? desiredBalance - deployerBalance : MULTICALL3_MIN_BALANCE - deployerBalance
      console.log(
      ...
      console.log(`   ↳ Canonical deployer already funded (${Value.formatEther(deployerBalance)} ETH)`)
    }

    console.log('   ↳ Publishing pre-signed Multicall3 deployment via cast...')
    const castResult = spawnSync('cast', ['publish', MULTICALL3_SIGNED_TX, '--rpc-url', RPC_URL], {
      stdio: 'inherit',
    })
    if (castResult.error) {
      throw new Error(`Failed to run "cast publish": ${castResult.error.message}`)
    }
    if (castResult.status !== 0) {
      throw new Error(`"cast publish" exited with status ${castResult.status}`)
    }

    const deployedMulticallCode = await publicClient.getBytecode({ address: multicall3Address })
    if (!deployedMulticallCode || deployedMulticallCode === '0x') {
      throw new Error('Failed to deploy Multicall3 at canonical address')
    }

    console.log(`   ✅ Multicall3 deployed at standard address: ${multicall3Address}`)
  }
  ...
  const chainsDir = resolve(import.meta.dirname, '../src/chains')
